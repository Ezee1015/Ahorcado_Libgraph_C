...
void Lee_Letra (char *Letra_Ingresada) {
    ...
    fflush(stdin);
//     printf("%c", *Letra_Ingresada);  
    fflush(stdin);    
    
}


void Comprobar_y_Colocar_Letra (char Letra_Ingresada, char *Palabra, int Cantidad_de_Letras, int Largo_Cada_Linea, int Espacio_por_cada_Linea, int *Total_Encontradas, int PosX_Ahorcado, int PosY_Ahorcado) {
    
    int Cant_Letras_Encontradas=0, i;
    
    for(i=0;i<=Cantidad_de_Letras;i++) {
    
        if(Palabra[i]==Letra_Ingresada) {
            Cant_Letras_Encontradas++;
            Imprimir_Letra(Palabra, i, Letra_Ingresada, Largo_Cada_Linea, Espacio_por_cada_Linea, PosX_Ahorcado, PosY_Ahorcado); //COMPLETAR ESTE LLAMADO DE FUNCION
        }
...
void Imprimir_Letra (char *Palabra, int Posicion, char Letra_Ingresada, int Largo_Cada_Linea, int Espacio_por_cada_Linea, int PosX_Ahorcado, int PosY_Ahorcado) {
    ...
    char Letra[120];
    sprintf(Letra,"%c",Letra_Ingresada); //COLOCA EN LA VARIABLE 'Letra' EL FORMATO CORRECTO PARA QUE OUTTEXTXY PUEDA IMPRIMIR CORRECTAMENTE 'Letra_Ingresada'
...
int main () {
    int gdriver=DETECT, gmode, i, Cantidad_de_Letras=0,Largo_Cada_Linea,Espacio_por_cada_Linea, Total_Encontradas=0;
    char Palabra[99], Letra_Ingresada='~';
    ...
    Comprobar_y_Colocar_Letra(Letra_Ingresada,&Palabra,Cantidad_de_Letras,Largo_Cada_Linea,Espacio_por_cada_Linea, &Total_Encontradas, 100/*POSICION 'x' DE LAS LINEAS DEL AHORCADO*/, 350/*POSICION 'y' DE LAS LINEAS DEL AHORCADO*/);
